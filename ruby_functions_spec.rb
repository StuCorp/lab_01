...
class Functions_Practice < MiniTest::Test
  ...
  def test_volume_of_cube()
    cube_result = volume_of_cube(2)
    cube_result2 = volume_of_cube(3)
    assert_equal(8, cube_result)
    assert_equal(27, cube_result2)
  end

  #Given the radius of a sphere calculate the volume
  def test_volume_of_sphere()
    sphere_volume = volume_of_sphere(2)
    assert_equal(33.51.round(), sphere_volume)
  end

  #Given a value in farenheit, convert this into celsius.
  def test_fahrenheit_to_celsius()
    temp = far_to_cel(32)
    second_temp = far_to_cel(-40)
    assert_equal(0, temp)
    assert_equal(-40, second_temp)
  end
